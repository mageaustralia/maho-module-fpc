...
class Mageaustralia_Fpc_Model_Warmup
{
    ...
    private function warmUrl(string $url, ?string $basicAuth): bool
    {
        try {
            $options = [
                'timeout' => 30,
                // Follow redirects: an FPC hit is served straight off disk (~2ms, no PHP), so
                // following a warm page costs almost nothing, and it robustly warms the final
                // destination of any stray redirect. The bulk redirect waste (historical renamed
                // url_key rewrites) is removed at the source in getProductRewritePaths(), so the
                // warm stream is cacheable URLs; this is just insurance for the odd one.
                'max_redirects' => 5,
                'headers' => [
